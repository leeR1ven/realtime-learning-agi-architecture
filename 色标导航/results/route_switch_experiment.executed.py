...
def paired_probe(checkpoint, *, seed, initial_route, max_steps, record_progress):
    ...
    for branch in BRANCHES:
        ...
        row.update(branch=branch, clone_state_sha256=clone_hash,
            ...
            strict_success=bool(row['correct_arrival'] and post_cue and
...
def main():
    ...
    report = {'status': 'running', 'protocol': {
        'birth_seed': 2026, 'mixed_neurons': 8192, 'max_events': 100000,
    ...
    if args.evaluate_only:
        ...
        brain = RouteSwitchController(seed=2026, mixed_neurons=8192, max_events=100000)
        ...
        for episode, (route, name, repeat) in enumerate(schedule):
            ...
            env = RouteSwitchEnv(seed=4000+episode)
